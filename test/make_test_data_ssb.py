...
with h5py.File(script_dir + "/data/test_potential_2d_ssb.h5", "r") as f:
    potential2d = f["data"][:]
...
print(f"dr = {dr}")
print(f"dk = {dk}")
potential = torch.as_tensor(energy2sigma(energy=E) * potential2d)
potential -= potential.min()
potential = torch.exp(1j * potential)
...
positions -= probe_tensor.shape[-2] // 2
...
# %%
fig, ax = vis.show_2d(torch.angle(batches[0, 120:130, :, :]), cbar=True, title="Data")
# %%
data = (
...
dataset = Dataset4dstem.from_array(
    array=torch.fft.fftshift(d, dim=(-2, -1)),
...
dataset.save(script_dir + "/data/test_data_ssb", mode="o", store="zip")
meta.to_yaml(script_dir + "/data/ssp_config_ssb.yaml")

print(f"Script directory: {script_dir}")

with h5py.File(script_dir + "/data/test_ssb_ground_truth.h5", "w") as f:
    f.create_dataset("phase_shift_function", data=potential.cpu().numpy())
    f.create_dataset("sampling", data=sampling)
    f.create_dataset("positions", data=positions.cpu().numpy())
    f.create_dataset("probe", data=probe_tensor.cpu().numpy())
