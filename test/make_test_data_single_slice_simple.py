...
with h5py.File(script_dir + "/data/test_potential_2d.h5", "r") as f:
    potential2d = f["data"][:]
...
# %%
fig, ax = vis.show_2d(torch.angle(batches[0, 28:34, :, :]), cbar=True, title="Data")
# %%
data = (
...
dataset.save(
    pathlib.Path(script_dir) / "/data/test_data_single_slice", mode="o", store="zip"
)
meta.to_yaml(script_dir + "/data/ssp_config.yaml")

print(f"Script directory: {script_dir}")

with h5py.File(script_dir + "/data/test_single_slice_ground_truth.h5", "w") as f:
